...
export default class KeysController extends Scene {
  ...
  update() {
    ...
    const { player, playerWeaponsGroup, enemies } = this.gameInstance;
    const { speed } = player; 
    ...
    if (Phaser.Input.Keyboard.DownDuration(this.keys.p, 1000) && debug) {
      ...
      const armaLampo = lampo.generazione(player.x, player.y, closestEnemy.x, closestEnemy.y, 1);
